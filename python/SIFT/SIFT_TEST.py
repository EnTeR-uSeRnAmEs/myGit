...
def GuassianKernel(sigma, dim):
    ...
    temp = 2 * sigma * sigma
    result = (1.0 / (temp * np.pi)) * \
        np.exp(-(assistant ** 2 + (assistant.T) ** 2) / temp)  # 二维高斯公式
    return result
...
def getDoG(img, n, sigma0, S=None, O=None):
    ...
    if O == None:
        # 计算最大可以计算多少层 O=log2（min(img长，img宽））-3
    ...
    DoG_Pyramid = [[Guass_Pyramid[o][s + 1] - Guass_Pyramid[o][s]
                    for s in range(S - 1)] for o in range(O)]  # 每一层中 上一张减去下一张得到高斯核
...
def LocateKeyPoint(DoG, sigma, GuassianPyramid, n, BinNum=36, contrastThreshold=0.04, edgeThreshold=10.0):
    ...
    for o in range(O):
        for s in range(1, S - 1):
            # 第一步：设定阈值
            threshold = 0.5 * contrastThreshold / \
                (n * 255 * SIFT_FIXPT_SCALE)  # 用于阈值化，去噪
            img_prev = DoG[o][s - 1]
            img = DoG[o][s]
            img_next = DoG[o][s + 1]
...
def SIFT(img, showDoGimgs=False):
    ...
    DoG, GuassianPyramid, octaves = getDoG(
        img, n, sigma0)  # 函数1.1，getDoG：得到高斯金字塔和高斯差分金字塔
    ...
    if showDoGimgs:
        ...
        plt.figure("Difference of Guassian")
        for i in range(octaves):
            for j in range(n + 2):
...
if __name__ == '__main__':
    origimg = plt.imread(r'1.jpg')  # 读第一张图片
    if len(origimg.shape) == 3:  # 如果是彩色图，就按照三通道取均值的方式转成灰度图
        img = origimg.mean(axis=-1)
    else:
        img = origimg
    # 用SIFT算法计算关键点（x坐标，y坐标，sigma，主方向，梯度幅值）和描述符（128维的向量）
    keyPoints, discriptors = SIFT(img, 1)

    origimg2 = plt.imread(r'2.jpg')  # 读第二张图片
    ...
    keyPoints2, discriptors2 = SIFT(img2, 1)  # 用SIFT算关键点和描述符
