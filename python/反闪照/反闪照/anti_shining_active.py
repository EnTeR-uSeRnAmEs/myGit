...
print(host)
SELF_ID = None
call_api = lambda api_name, **data: loads(get(
    host['http']+api_name+'?'+'&'.join(f"{k}={v}" for k, v in data.items())).content)
...
def on_message(ws, message):
    ...
    if data['post_type'] == 'meta_event':
        ...
        if 'type=flash' in data['message']:
            ...
            res = call_api("send_msg", user_id=SELF_ID,
                           message=f"来自{data['sender']['nickname']}的闪照[CQ:image,file={url}]")
...
def start_listening():
    ws = websocket.WebSocketApp(
        host['ws'], on_message=on_message, on_error=on_error, on_close=on_close)
    while True:
        try:
            ws.run_forever()
